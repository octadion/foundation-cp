...
embedded. A class with none therefore has no position of its own, and the fall-backs differ only in
where it is placed: at the extreme of the range, in a null cluster, or at an infinite
threshold. Descriptors read from the classifier's weights have no such gap, which is the
difference \cref{tab:competitors} measures.
...
coverage gap. Both remain defined for a class with no calibration examples, and both draw their
per-class signal from one scalar. Prevalence alone is significantly worse than geometry
(\cref{tab:ablation}) and is beaten by a random projection of the same dimension
(\cref{tab:competitors}).
...
Both need a density ratio or online label feedback, which our setting does not provide, so
we read our corrupted-input experiment as a stress test and make no coverage claim there
(\cref{app:imagenetc}).
