...
\paragraph{Metrics.} \Cref{tab:metrics} in the appendix defines every quantity we report.
The headline statistic is worst-class coverage, since a single badly covered class is what a
...
reaches $0.027$ and six of the eight change the statistic by exactly zero. Paired over
splits, PCC exceeds those six with intervals excluding zero; against the two moving variants
of \textsc{Fuzzy} CP the differences are positive, at $0.049$ and $0.072$, but their intervals
include zero at three splits. Three aspects of the table need comment.
...
\paragraph{Methods that are defined here and still do not move.} Three of the eight need no
per-class calibration examples, so their zeros are measurements and not fallbacks.
\textsc{Clustered} CP places every held-out class in its null cluster, which is the marginal
threshold by construction. \textsc{Interp-Q} is the more interesting case. Its rule for an
undefined classwise quantile is to replace it with one, the largest value the softmax score
can take, and then blend; a held-out class therefore receives the widest threshold in the
label space, and once average set size is matched back down every other class pays for it.
Sweeping the interpolation weight over $\{0,0.5,0.9,0.99,0.999,1\}$ and keeping the best, the
selected weight is zero on every split, at which point the method is \textsc{Standard} CP. The
construction that makes it defined at $n_y=0$ is the same one that makes it unhelpful there.
...
\Cref{tab:depth} sweeps the score function and $\alpha$. The correction survives every score
but is strongest with LAC and attenuates for the adaptive ones. A plausible mechanism is that
APS, RAPS and SAPS already reshape the score using rank or cumulative mass, so part of the
class-difficulty signal that $\delta_y$ captures has been absorbed into the score itself. At
$\alpha=0.01$ the effect is exactly zero, which is consistent with the per-class offsets
becoming indistinguishable from noise as the quantile moves into the extreme tail.

The prevalence-adjusted score of \citet{ding2026conformal} is included because it is the other
published construction that remains defined for a class with no calibration examples, and
because a reader may reasonably ask whether it removes the need for a per-class correction. It
does not, though the reading is not a clean win either: on PAS scores our improvement is
$0.066$ with an interval that contains zero, and the oracle ceiling rises to $0.234$, so PAS
opens more room than LAC does and we take a smaller share of it, $28\%$ against $42\%$. Both
observations are consistent with prevalence carrying real but coarse information about class
difficulty, which is what \cref{tab:ablation} finds from the descriptor side.
