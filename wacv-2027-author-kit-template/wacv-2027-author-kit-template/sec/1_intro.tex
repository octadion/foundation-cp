...
\caption{(a) What existing methods can offer a class whose calibration set is empty, and what
PCC offers instead. (b) The three steps: the offset is observed only where labels exist,
regressed on descriptors read from the classifier's weights, then shrunk and recalibrated.
Schematic; measured behaviours are in \cref{tab:competitors}.}
\label{fig:overview}
\end{figure}
...
iNaturalist-2018 \cite{vanhorn2018inaturalist} it has two. \Cref{fig:overview}(a) and \cref{tab:methods} summarise
what existing methods do with such a class. \textsc{Classwise} CP and \textsc{Rc3p} return an
infinite threshold, so the prediction set becomes the entire label space. \textsc{Clustered} CP
places the class in a null cluster and falls back to the shared marginal threshold. The
kernel-weighted method of \citet{ding2026conformal} embeds each class by its own calibration
scores, so a class with none collapses to a single point of the embedding and every such class
receives the same threshold. None of this is unreasonable, since with no labelled example of a
class there is nothing in the calibration set to condition on.
