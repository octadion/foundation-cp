...
\TODO{Fill from phase R of notebook 13: marginal coverage at the emitted thresholds against
the $1-\alpha$ target for all three arms, and $\Delta$ worst-class paired over the ten splits.
CPU-only.}
...
\TODO{Fill from phase M of notebook 13, which sweeps the budget over $250$k, $500$k and $750$k
rows at five splits each, giving $174$, $349$ and $525$ calibration examples per class and
$75$, $150$ and $225$ evaluation examples. Report $\Delta$ worst-class at each budget with its
interval, and the slope against $\log$ rows. If the slope is positive the prediction above
holds and the reported numbers are conservative; if it is negative the headline depends on the
subsample and that belongs in \cref{sec:discussion} rather than here. The full dump is not in
the sweep because its peak footprint is $9.7$\,GB.}
