...
\section{What each method gives a class with no data}
\label{app:methods}

\Cref{tab:methods} lists the per-class threshold rules compared in this paper and what each
one produces when $n_y=0$. Three of them stay defined there without per-class labels, and
\cref{sec:comp} reports what that is worth.
...
\begin{tabular}{@{}lll@{}}
\toprule
Method & $q_y$ built from & If $n_y=0$ \\
\midrule
\textsc{Standard} \cite{vovk2005algorithmic}  & shared quantile   & shared \\
\textsc{Classwise} \cite{sadinle2019least}    & own quantile      & $\infty$ \\
\textsc{Clustered} \cite{ding2023class}       & cluster quantile  & shared \\
\textsc{Rc3p} \cite{shi2024rank}              & quantile + rank   & $\infty$ \\
\textsc{Fuzzy} \cite{ding2026conformal}       & kernel weights    & shared \\
\textsc{Pas} \cite{ding2026conformal}         & shared quantile   & shared \\
\textsc{Interp-Q} \cite{ding2026conformal}    & blend of the two  & capped \\
\textsc{Tacp} \cite{liu2026tail}              & head penalty      & by group \\
\midrule
\textbf{PCC} (ours) & $g_\theta(\varphi(y))$ & \textbf{predicted} \\
\bottomrule
\end{tabular}
...
Worst-class (A)
  & $\min_{y}\hat c_y$ \\[2pt]
Bin-worst (B)
  & $\min_{b}\hat c^{(b)}$, prevalence bins \\[2pt]
$\Delta$ worst
  & worst-class minus marginal \\[2pt]
Below target
  & $\frac{1}{|\mathcal{Y}|}\sum_y\mathbf{1}\{\hat c_y<1-\alpha\}$ \\[2pt]
SSCV
  & $\max_{b}|\hat c^{(b)}-(1-\alpha)|$, strata \\[2pt]
Worst-slab
  & $\min$ mean $\hat c_y$ over windows \\[2pt]
Set size
...
\bottomrule
\end{tabular}
\caption{Evaluation metrics. $\hat c_y$ is the empirical coverage of class $y$, and regimes A and B are the per-class and prevalence-binned cases. $J_y$ is the set of test indices with label $y$ and $N$ the
...
\Cref{tab:ablation} is the evidence for \cref{sec:ablations}. Each row removes one component
over ten ImageNet splits at $\alpha=0.10$, and a positive value means the full method is
better than the ablated one.
...
\bottomrule
\end{tabular}
\caption{Ablations on ImageNet at $\alpha=0.10$, ten splits. Positive means the full method is
better. $d_z$ is Cohen's paired effect size; the last column marks Holm-corrected significance
at $0.05$ \cite{holm1979simple}. The last two rows are identical to the full method at matched
set size by construction (\cref{app:invisible}).}
\label{tab:ablation}
\end{table}
...
calibration. \Cref{fig:sweeps}a places $35$ below the depth at which any effect is
...
\begin{tabular}{@{}llc@{}}
\toprule
Backbone & $\varphi$ & $\Delta$ worst-class \\
\midrule
ConvNeXt-T \cite{liu2022convnet}     & own   & $+0.000\ [-0.000,+0.000]$ \\
                                     & fixed & $+0.011\ [-0.017,+0.040]$ \\
ResNet-50 \cite{he2016deep}          & own   & $+0.017\ [-0.005,+0.040]$ \\
                                     & fixed & $+0.006\ [-0.005,+0.017]$ \\
ViT-B/16 \cite{dosovitskiy2021image} & own   & $-0.029\ [-0.072,+0.015]$ \\
                                     & fixed & $-0.011\ [-0.066,+0.043]$ \\
\bottomrule
\end{tabular}
...
\begin{tabular}{@{}llcc@{}}
\toprule
Dataset & Thr. & Kept & $\Delta$ worst-class \\
\midrule
Pl@ntNet  & $\ge 35$ & $151$ & $+0.020\ [-0.011,+0.052]$ \\
Pl@ntNet  & $\ge 75$ & $98$  & $\mathbf{+0.019}\ [+0.010,+0.028]$ \\
iNat-2018 & $\ge 35$ & $237$ & $+0.000$ \\
iNat-2018 & $\ge 75$ & $75$  & $+0.000$ \\
\bottomrule
\end{tabular}
...
and not at the looser one, which is the ordering \cref{fig:sweeps}a predicts, since $35$ sits
...
eight times larger, fitted in a descriptor space of the same dimension. A richer regression
head does not recover it: the quadratic and kernel heads of \cref{app:heads} abstain in all
five splits here, exactly as the ridge does. Whether richer descriptors would is untested, and
...
\midrule
worst & $\mathbf{0.699}$ & $0.466$ & $0.385$ & $0.350$ & $0.319$ \\
\bottomrule
\end{tabular}
...
\toprule
Corruption & Sev. & $\Delta$ & Cov. & Size & Empty \\
\midrule
...
\section{What the regression fits}
\label{app:fit}

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{figures/fig_method.pdf}
\caption{The mechanism, on the primary ImageNet configuration. Every labelled class
contributes an observed offset $\delta_y$ and a predicted one $g_\theta(\varphi(y))$; held-out
classes appear as ticks on the horizontal axis, because the observed coordinate does not exist
for them. The regression recovers the direction of the correction and understates its
magnitude, and the classes that decide the minimum lie in the dense band near $\delta_y=0$,
where the cost is prediction error rather than shrinkage towards the mean. That error is the
quantity \cref{prop:sup} bounds, and the reason $\hat\lambda=0.10$ and not $1$.}
\label{fig:method}
\end{figure}
...
Worst-class coverage is a minimum, and a minimum says nothing about the shape it was taken
from. \Cref{fig:coverage} therefore plots the whole sorted curve and \cref{tab:whohelped}
reports the mean change by prevalence quintile, with the fraction of classes that rise and
fall. Both come from the first split of the primary configuration, the only one for which we
stored the per-class breakdown.

\begin{figure*}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig_coverage.pdf}
\caption{Class-conditional coverage of every class, sorted, on the primary ImageNet
configuration. Rank rather than percentile on the horizontal axis, because worst-class
coverage is decided by the leftmost few classes and a linear axis compresses them to nothing.
The correction rotates the curve: the tail rises and the classes that had coverage to spare
give some back, with the crossing near rank $150$ of $300$ on the held-out classes. The oracle
performs the same rotation further, which is the sense in which it is a ceiling rather than a
different kind of method.}
\label{fig:coverage}
\end{figure*}

The curve rotates rather than shifting. On the held-out classes the lowest-ranked class rises
from $0.658$ to $0.691$ while the median falls from $0.921$ to $0.909$, and macro-coverage moves
from $0.904$ to $0.900$. The oracle rotates the same curve further, lifting its worst class to
$0.792$ and pulling its median to $0.883$; what separates the two is how far the rotation goes,
not which way. Because the comparison holds average set size fixed, coverage given to the
classes that were failing has to come from somewhere, and it comes in thin slices from the
classes that had it to spare. Whether that trade is worth making is a decision about the
application rather than about the method, which is why we report both the minimum and the
average throughout.

Two cautions on reading the fractions in \cref{tab:whohelped}. At the $76$ evaluation examples
per class of this configuration a single example is $0.013$ of coverage, so how many classes
register as changed at all depends on evaluation depth as much as on the correction; the mean
change of $-0.004$ is the quantity that survives a change of depth. And the worst class is not
the same class before and after, class $754$ before the correction and class $663$ after, so the
two ends of that arrow are not one class being repaired.

Prevalence carries little information here, because ImageNet's held-out classes hold between
$693$ and $1199$ rows in the score dump and are therefore close to balanced. The quintiles are
included so that the same table can be read for the long-tailed datasets, where prevalence
...
\begin{tabular}{@{}lccc@{}}
\toprule
Quintile & Classes & Med.\ prev. & Mean $\Delta\hat c_y$ \\
\midrule
1, frequent & $60$ & $1182$ & $-0.0078$ \\
2 & $60$ & $1174$ & $-0.0033$ \\
3 & $60$ & $1169$ & $-0.0033$ \\
4 & $60$ & $1163$ & $-0.0039$ \\
5, rare & $60$ & $1147$ & $-0.0024$ \\
\midrule
\multicolumn{4}{@{}l@{}}{Improved $11.3\%$, worsened $43.3\%$} \\
\multicolumn{4}{@{}l@{}}{Worst class $0.658 \rightarrow 0.691$} \\
\bottomrule
\end{tabular}
...
\midrule
$\hat q$ \eqref{eq:qglobal}      & calib. & labelled \\
$\delta_y$ \eqref{eq:delta}      & calib. & labelled, $n_y\ge n_{\mathrm{cal}}$ \\
$g_\theta$                       & calib. & labelled, $n_y\ge n_{\mathrm{cal}}$ \\
$\lambda$ \eqref{eq:shrink}      & calib. & labelled \\
$n_{\mathrm{cal}}$               & calib. & labelled \\
$c$ \eqref{eq:set}               & calib. & labelled \\
\midrule
\emph{all reported numbers}      & \emph{eval.} & \emph{both tables} \\
\bottomrule
\end{tabular}
...
\begin{tabular}{@{}lccc@{}}
\toprule
Variant & Marg.\ cov. & Deficit & $\Delta$ worst \\
\midrule
Reuse         & $0.8994$ & $-0.0018$ & $+0.0588$ \\
Split $25\%$  & $\mathbf{0.9011}$ & $\mathbf{-0.0000}$ & $+0.0636$ \\
Split $50\%$  & $0.9003$ & $-0.0007$ & $+0.0585$ \\
\midrule
Target        & $0.9000$ & ---       & --- \\
\bottomrule
\end{tabular}
...
\cref{fig:sweeps}a shows the measurable effect rising with evaluation depth. A larger budget
...
same effect \cref{fig:sweeps}a measures from the other direction.
...
\begin{tabular}{@{}rcclc@{}}
\toprule
Rows & Cal. & Eval. & $\Delta$ worst-class & Ceil. \\
\midrule
...
$250$k row reproduces the corresponding entry of \cref{fig:sweeps}b to four decimal places,
...
\begin{tabular}{@{}llc@{}}
\toprule
Descriptors & $\Delta$ worst-class & CV err. \\
\midrule
Cosine, all       & $+0.058\ [+0.006,+0.110]$ & $0.036$ \\
Euclidean         & $+0.045\ [+0.009,+0.081]$ & $0.040$ \\
No head bias      & $+0.058\ [+0.006,+0.110]$ & $0.036$ \\
$k\in\{1,2,5\}$   & $+0.058\ [+0.006,+0.110]$ & $0.036$ \\
$k\in\{1,10,20,100\}$ & $+0.063\ [+0.016,+0.111]$ & $0.036$ \\
\bottomrule
\end{tabular}
...
\section{The regression head}
\label{app:heads}

\Cref{sec:correction} fits $g_\theta$ by ridge regression. Whether that linear form is what
limits the method is a fair question, and it is sharpest on iNaturalist-2018, where the
regression carries no usable signal at all. We replaced the ridge with two closed-form
nonlinear heads and changed nothing else: a \emph{quadratic} head, a ridge over the squares
and pairwise products as well as the features, and \emph{kernel} ridge with an RBF kernel.
Both are shrunk, selected and recalibrated by the same procedure as the linear head, so the
rows of \cref{tab:heads} are comparable with the rest of the paper. The class floor is checked
against the coefficients each head actually fits, which for the quadratic head over $p=9$
features is $54$ rather than $9$.

The linear head is not the constraint. On ImageNet the quadratic head moves the point estimate
by $0.0004$ and widens the interval to include zero, beating the linear head on one of the five
splits, tying on two and losing on two; its standard deviation across splits is $0.079$ against
the linear head's $0.059$. The kernel head is worse at every width we tried, and at
$\gamma=3$ it is worse than applying no correction.

On the long-tailed datasets the reason is visible rather than inferred. Pl@ntNet-300K is where
the extra capacity ought to pay, since there is a real effect to find, and there the shrinkage
step declines to use the nonlinear predictions: $\hat\lambda=0$ in three of five splits for
both nonlinear heads and in none of them for the linear head. On iNaturalist-2018 every head
abstains in every split, so what fails at $K=8142$ is not the functional form of $g_\theta$.
The oracle ceiling there is $0.018$, and that is the more immediate obstacle.

The direction is the one \cref{prop:sup} describes. Worst-class coverage is controlled by the
largest error in the predicted offsets, and a head with more capacity fits the labelled classes
more closely without any corresponding control on its worst error over classes it has never
seen. We did not measure that sup-norm error directly, so this is an interpretation and not a
second measurement; what is measured is that the extra capacity buys no point estimate, costs
interval width on every dataset, and is declined outright wherever the shrinkage step is free
to decline it.
...
\begin{tabular}{@{}llccc@{}}
\toprule
Data & Head & $\Delta$ worst-class & $\bar\lambda$ & $\hat\lambda{=}0$ \\
\midrule
\multirow{5}{*}{\shortstack[l]{Image\\Net}}
 & linear    & $\mathbf{+0.058}\ [+0.006,+0.110]$ & $0.10$ & $0/5$ \\
 & quadratic & $+0.058\ [-0.011,+0.128]$ & $0.20$ & $0/5$ \\
 & kernel    & $+0.026\ [+0.008,+0.045]$ & $0.32$ & $0/5$ \\
 & \ \ $\gamma{=}0.3$ & $+0.046\ [+0.004,+0.088]$ & $0.23$ & $0/3$ \\
 & \ \ $\gamma{=}3$   & $-0.022\ [-0.065,+0.021]$ & $0.27$ & $0/3$ \\
\midrule
\multirow{3}{*}{\shortstack[l]{Pl@nt\\Net}}
 & linear    & $\mathbf{+0.019}\ [+0.010,+0.028]$ & $0.22$ & $0/5$ \\
 & quadratic & $+0.014\ [-0.009,+0.038]$ & $0.06$ & $3/5$ \\
 & kernel    & $+0.009\ [-0.005,+0.023]$ & $0.04$ & $3/5$ \\
\midrule
iNat & all & $+0.000\ [0.000,0.000]$ & $0.00$ & $5/5$ \\
\bottomrule
\end{tabular}
\caption{Replacing the ridge $g_\theta$ with a nonlinear head, everything else at the
configuration of the corresponding table. Five splits, except the two indented RBF-width rows,
which are three and carry their own ceiling of $0.153$. Unindented kernel rows use $\gamma=1$.
Oracle ceilings are $0.140$ on ImageNet, $0.086$ on Pl@ntNet-300K under the $75$-example
restriction of \cref{tab:longtail}, and $0.018$ on iNaturalist-2018. The last column counts
splits in which the shrinkage step declined the prediction entirely.}
\label{tab:heads}
\end{table}
