...
call $\bar\delta = \delta + \sigma(\delta)$ the \emph{matched} offset. This is the
resource-matched comparison used throughout \cref{sec:exp}.
...
$\delta$ well, and the method works exactly as far as the regression does. Across our three
...
improvements of $0.058$, $0.019$ and $0.000$. Because \cref{eq:lambda} can return
$\hat\lambda=0$, a poor regression is abstained from and not acted on, which is what happens
on iNaturalist-2018 in every split (\cref{sec:depth}).
