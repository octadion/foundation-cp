...
are free of \emph{calibration} labels, though not of labels altogether. Nothing depends on
prevalence. Removing it costs $0.011$ in worst-class coverage without reaching significance
($p=0.30$),
while prevalence alone is significantly worse than the full set (\cref{tab:ablation}). The
...
Shrinkage is what makes the method usable. Worst-class coverage is governed by the largest
error in $\tilde\delta$, which mean squared error does not control, so a predictor with a
respectable $R^2$ can still be harmful at full strength: $\lambda=1$ costs $0.584$ relative to
the selected $\lambda\approx0.1$, while $\lambda=0$ recovers the marginal threshold exactly
(\cref{tab:ablation}).
...
(\cref{sec:depth}). A practitioner can therefore read $\lambda$ as a diagnostic, since it reports
how much of $g_\theta$ the calibration data was willing to trust.
